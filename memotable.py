...
def mkmemotable(self): #self == frame
	if 	self.parent.EDF != None:
		...
		self.parent.csvpath = self.parent.dir+'/anno/'+'memo'+self.parent.edfname[:-4]+'.csv'
...
def btnlist(self):
	...
	self.parent.addbtn.setFixedSize(60,25)
	self.parent.delbtn = QPushButton('Delete')
	self.parent.delbtn.setFixedSize(60,25)
	...
	self.parent.savebtn.setFixedSize(60,25)
	self.parent.closebtn = QPushButton('Close')
	self.parent.closebtn.setFixedSize(60,25)
...
def btnfunc(self):
	...
	def saveF(frame):
		...
		try:
			df.to_csv('./anno/'+'memo'+self.parent.edfname[:-4]+'.csv',mode='w',index=False,encoding="euc-kr")
		except:
